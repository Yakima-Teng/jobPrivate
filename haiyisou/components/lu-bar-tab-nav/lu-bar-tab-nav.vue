...
<view :id="'luBarTabNav'+barId" class="lu-bar-tab-nav">
  <view class="lu-tab-box">
      <scroll-view v-if="!!barFixed" id="luTabStatic" :style="{ top: barTopStyles,height:barHeightStyles }" class="lu-bar-tab lu-bar-tab-fixed ss-scroll-navbar" scroll-x :scroll-left="scrollLeft" scroll-with-animation="true" >
        <view
          v-for="(item, index) in tabList" 
          :key="index" 
          class="lu-tab-item"
          :class="[selectedIndex==index? 'lu-active' : '',!!iconShow? 'lu-icon-show' : '']"
          @click="_scrollToTarget(index)"
          :id="'item-' + index"
          >
...
<script>
  // import ssScrollNavbar from '@/components/ss-scroll-navbar/ss-scroll-navbar'
	export default {
    components:{
      // ssScrollNavbar
    },
...
.lu-bar-tab-nav {	position: relative; width: 100%; background-color: #fff; 
	.lu-tab-box { background-color: #ebf2fe; padding:0 40rpx; position: sticky; width: 100%; top:15rpx + 180rpx; box-sizing: border-box;  z-index: 10;}
...
.ss-scroll-navbar { white-space:nowrap; }  
