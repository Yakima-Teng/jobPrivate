<template>
	<view class="page-danger-info">
		    <view class="danger-info-list">
		    	<view class="danger-info" v-if="isShow">
					<view class="info" >
						<text class="title">编号：</text>
						<text class="txt">{{MeasureData.accidentNo}}</text>
					</view>
					<view class="info" >
						<text class="title">模型名称：</text>
						<text class="txt">{{MeasureData.accidentName}}</text>
					</view>
					<view v-for="(item,index) in xqList" :key="index">
						<view class="info">
							<text class="title">{{item.accidentTypeName}}：</text>
							<text class="txt"><B>{{item.accidentDetailName}}</B></br>{{item.accidentDetailContent}}</text>
							</view>
					</view>
		    	</view>
		    </view>
...
export default {
  ...
  onLoad(options){
	...
	methods: {
		...
		getSgyjxq(code){
			//请求后台数据
			this.api.requestNoLoading('/dg/accEme/getData?code='+code,{},'GET')
			.then(res => {
