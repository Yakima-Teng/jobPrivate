<template>
 <div class="container">
     <div v-for='(item,index) in orderData' :key="index">
        <div class="order-c"   v-if="item.goods.length==1">
            ...
            <ul class="order-d clearfix">
                ...
                <li>
                    ...
                    <p class="ful" v-if='(item.goods[0].price!=item.goods[0].pay_price)&&(item.goods[0].order_state=="等待付款"||item.goods[0].order_state=="已取消")'>应付:￥{{item.goods[0].pay_price}}</p>
                    <p class="ful" v-if='(item.goods[0].price!=item.goods[0].pay_price)&&(item.goods[0].order_state=="已完成"||item.goods[0].order_state=="办理中")'>实付:￥{{item.goods[0].pay_price}}</p>
                </li>
                <li>
                    <p class="susful" v-if="item.goods[0].order_state=='已完成'">{{item.goods[0].order_state}}</p>
                    <p  class="ful" v-if="item.goods[0].order_state=='等待付款'">{{item.goods[0].order_state}}</p>
                    <p  class="ful2" v-if="item.goods[0].order_state=='办理中'">{{item.goods[0].order_state}}</p>
                    <p  class="susful" v-if="item.goods[0].order_state=='已取消'">{{item.goods[0].order_state}}</p>
                    <p @click="openUrl(item.goods[0].goods_id,2)">订单详情</p>
                </li>
                <li v-if="item.goods[0].order_state=='等待付款'">
                    <div>
                ...
                <li v-if="item.goods[0].order_state=='办理中'">
                    <button class="isagainC" @click="again(item.goods[0])" v-show="item.goods[0].confirm_type==0">确认服务</button>
                    <div :class="{isagainC:item.goods[0].confirm_type==1}">订单跟踪 <div class="track-f"><trackhandle :dataT="item.goods[0].schedule" type='schedule'></trackhandle></div></div>
                </li>
                <li v-if="item.goods[0].order_state=='已完成'">
                    <button class="again isagainC" @click="purchase(2,item.goods[0].child_order_num)">再次购买</button>
                </li>
                <li v-if="item.goods[0].order_state=='已取消'">
                    <button class="again isagainC" @click="purchase(2,item.goods[0].child_order_num)">再次购买</button>
                </li>
            </ul>
        </div>
        <div class="order-c order-f"  v-if="((item.order_state==2)||(item.order_state==3))&&item.goods.length>1">
            <div class="order-t">
                <p class="clearfix">
            ...
            <ul class="order-d clearfix" v-for='(el,indexEL) in item.goods' :key="indexEL">
                <li class="clearfix">
                    <p class="order-t order-ft clearfix">
                ...
                <li>
                    <p class="susful" v-if="el.order_state=='已完成'">{{el.order_state}}</p>
                    <p  class="ful" v-if="el.order_state=='等待付款'">{{el.order_state}}</p>
                    <p class="ful2" v-if="el.order_state=='办理中'">{{el.order_state}}</p>
                    <p class="susful" v-if="el.order_state=='已取消'">{{el.order_state}}</p>
                    <p @click="openUrl(el.goods_id,2)">订单详情</p>
                </li>
                <li v-if="el.order_state=='等待付款'">
                    <div>
                ...
                <li v-if="el.order_state=='已完成'">
                    <button class="again" @click="purchase(2,el.child_order_num)">再次购买</button>
                </li>
                <li v-if="el.order_state=='已取消'">
                    <button class="again" @click="purchase(2,el.child_order_num)">再次购买</button>
                </li>
                <li v-if="el.order_state=='办理中'">
                    <button class="again2" @click="again(el)" v-show="el.confirm_type==0">确认服务</button>
        ...
        <div class="order-c" v-if="((item.order_state==1)||(item.order_state==4))&&(item.goods.length>1)">
            <div class="order-t">
                <p class="clearfix">
                    <span class="o-time">{{item.create_time}}</span>
                    <span class="o-num">订单号:
            ...
            <div class="order-d clearfix" v-for="(el,el_index) in item.goods" :key='el_index'>
                <ul class="clearfix" v-if="el_index==0">
                    ...
                    <li class="box3 box5">
                        <p class="ful" v-if="item.order_state==1">等待付款</p>
                        <p class="susful" v-if="item.order_state==4">已取消</p>
                    ...
                    <li class="box4 box5">
                        ...
                        <button class="again isagainC" @click="purchase(1,item.orderNumber)" v-if="item.order_state==4">再次购买</button>
                    </li>
                </ul>
                <ul class="clearfix" v-if="el_index>0" :class="{box6:item.goods.length==el_index+1}">
                    <li class="clearfix box1" >
                        <div class="order-z clearfix">
                            <div @click="togoods(el)">
                                <img :src="el.img" alt="">
...
export default {
 ...
 methods: {
     ...
     purchase(type,orderNumber){
        ...
        //         }
        //     })
        Api.postorderBuyAgain(type,orderNumber).then(res=>{
            console.log(res);
            if(res.data.code==200){
